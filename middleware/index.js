...
/**
 * Express middleware to verify hmac and requests from Haravan.
...
function verifyWebhook(req, res, next) {
  ...
  let data;
  try {
    hmac = req.get('X-Haravan-Hmac-SHA256');
    data = req.body;
  } catch (e) {
    console.log(`Webhook request failed from: ${req.get('X-Haravan-Shop-Domain')}`);
    res.sendStatus(200);
  }

  if (verifyHmac(JSON.stringify(data), hmac)) {
    req.topic = req.get('X-Haravan-Topic');
    req.shop = req.get('X-Haravan-Shop-Domain');
    return next();
  }
